...
var ShadyCSS = {
  ...
  _setElementClassViaClassList: function _setElementClassViaClassList(element, classString) {
  ...
  _setElementClassViaAttr: function _setElementClassViaAttr(element, classString) {
    var root = element.getRootNode();
    var scopeName = root.host && root.host.localName;
    var classes = [];
    ...
    if (classString) {
      var k$ = classString.split(/\s/);
      for (var i = 0; i < k$.length; i++) {
        var k = k$[i];
        if (k === StyleTransformer.SCOPE_NAME || k === StyleProperties.XSCOPE_NAME) {
          i++;
        } else {
          classes.push(k);
        }
    ...
    if (!scopeName) {
      ...
      if (classAttr) {
        var _k$ = classAttr.split(/\s/);
        for (var _i = 0; _i < _k$.length; _i++) {
          if (_k$[_i] === StyleTransformer.SCOPE_NAME) {
            scopeName = _k$[_i + 1];
            break;
          }
        }
