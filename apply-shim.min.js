...
function v(a){a=a.replace(aa,"").replace(ba,"");var b=w,c=a,e=new u;e.start=0;e.end=c.length;for(var d=e,f=0,h=c.length;f<h;f++)if("{"===c[f]){d.rules||(d.rules=[]);var g=d,k=g.rules[g.rules.length-1]||null;d=new u;d.start=f+1;d.parent=g;d.previous=k;g.rules.push(d)}else"}"===c[f]&&(d.end=f+1,d=d.parent||e);return b(e,a)}
...
function C(a,b,c){c=void 0===c?"":c;var e="";if(a.cssText||a.rules){var d=a.rules,f;if(f=d)f=d[0],f=!(f&&f.selector&&0===f.selector.indexOf("--"));if(f){f=0;for(var h=d.length,g;f<h&&(g=d[f]);f++)e=C(g,b,e)}else b?b=a.cssText:(b=a.cssText,b=b.replace(ea,"").replace(fa,""),b=b.replace(ha,"").replace(ia,"")),(e=b.trim())&&(e="  "+e+"\n")}e&&(a.selector&&(c+=a.selector+" {\n"),c+=e,a.selector&&(c+="}\n\n"));return c}
var B=1,z=7,y=4,A=1E3,aa=/\/\*[^*]*\*+([^/*][^*]*\*+)*\//gim,ba=/@import[^;]*;/gim,ea=/(?:^[^;\-\s}]+)?--[^;{}]*?:[^{};]*?(?:[;\n]|$)/gim,fa=/(?:^[^;\-\s}]+)?--[^;{}]*?:[^{};]*?{[^}]*?}(?:[;\n]|$)?/gim,ha=/@apply\s*\(?[^);]*\)?\s*(?:[;\n]|$)?/gim,ia=/[^;:]*?:[^;]*?var\([^;]*\)(?:[;\n]|$)?/gim,da=/^@[^\s]*keyframes/,x=/\s+/g;var D=/(?:^|[;\s{]\s*)(--[\w-]*?)\s*:\s*(?:((?:'(?:\\'|.)*?'|"(?:\\"|.)*?"|\([^)]*?\)|[^};{])+)|\{([^}]*)\}(?:(?=[;\s}])|$))/gi,F=/(?:^|\W+)@apply\s*\(?([^);\n]*)\)?/gi,ja=/@media\s(.*)/;var G=new Set;function H(a){if(!a)return"";"string"===typeof a&&(a=v(a));return C(a,t)}function I(a){!a.__cssRules&&a.textContent&&(a.__cssRules=v(a.textContent));return a.__cssRules||null}function J(a,b,c,e){if(a){var d=!1,f=a.type;if(e&&f===y){var h=a.selector.match(ja);h&&(window.matchMedia(h[1]).matches||(d=!0))}f===B?b(a):c&&f===z?c(a):f===A&&(d=!0);if((a=a.rules)&&!d){d=0;f=a.length;for(var g;d<f&&(g=a[d]);d++)J(g,b,c,e)}}}
...
P.prototype.m=function(a,b){if(void 0===a.b){var c=[];for(var e=a.content.querySelectorAll("style"),d=0;d<e.length;d++){var f=e[d];if(f.hasAttribute("shady-unscoped")){if(!l){var h=f.textContent;G.has(h)||(G.add(h),h=f.cloneNode(!0),document.head.appendChild(h));f.parentNode.removeChild(f)}}else c.push(f.textContent),f.parentNode.removeChild(f)}(c=c.join("").trim())?(e=document.createElement("style"),e.textContent=c,a.content.insertBefore(e,a.content.firstChild),c=e):c=null;a.b=c}return(a=a.b)?this.j(a,
...
function ma(a,b,c){b=b.replace(D,function(b,d,f,h){return na(a,b,d,f,h,c)});return Q(a,b,c)}function oa(a,b){for(var c=b;c.parent;)c=c.parent;var e={},d=!1;J(c,function(c){(d=d||c===b)||c.selector===b.selector&&Object.assign(e,R(a,c.parsedCssText,!1))});return e}
function Q(a,b,c){for(var e;e=F.exec(b);){var d=e[0],f=e[1];e=e.index;var h=b.slice(0,e+d.indexOf("@apply"));b=b.slice(e+d.length);var g=c?oa(a,c):{};Object.assign(g,R(a,h,!1));d=void 0;var k=a;f=f.replace(ka,"");var n=[];var m=k.a.get(f);m||(k.a.set(f,{}),m=k.a.get(f));if(m){k.c&&(m.i[k.c]=!0);var q=m.h;for(d in q)k=g&&g[d],m=[d,": var(",f,"_-_",d],k&&m.push(",",k.replace(M,"")),m.push(")"),M.test(q[d])&&m.push(" !important"),n.push(m.join(""))}d=n.join("; ");b=h+d+b;F.lastIndex=e+d.length}return b}
function R(a,b,c){c=void 0===c?!0:c;b=b.split(";");for(var e,d,f={},h=0,g;h<b.length;h++)if(e=b[h])if(g=e.split(":"),1<g.length){e=g[0].trim();d=g.slice(1).join(":");if(c){var k=a;g=e;var n=la.exec(d);n&&(n[1]?(k.b||(k.b=document.createElement("meta"),k.b.setAttribute("apply-shim-measure",""),k.b.style.all="initial",document.head.appendChild(k.b)),g=window.getComputedStyle(k.b).getPropertyValue(g)):g="apply-shim-inherit",d=g)}f[e]=d}return f}function pa(a,b){if(O)for(var c in b.i)c!==a.c&&O(c)}
function na(a,b,c,e,d,f){e&&K(e,function(b,c){c&&a.a.get(c)&&(d="@apply "+c+";")});if(!d)return b;var h=Q(a,""+d,f);f=b.slice(0,b.indexOf("--"));var g=h=R(a,h),k=a.a.get(c),n=k&&k.h;n?g=Object.assign(Object.create(n),h):a.a.set(c,g);var m=[],q,X=!1;for(q in g){var E=h[q];void 0===E&&(E="initial");!n||q in n||(X=!0);m.push(c+"_-_"+q+": "+E)}X&&pa(a,k);k&&(k.h=g);e&&(f=b+";"+f);return f+m.join("; ")+";"}P.prototype.detectMixin=P.prototype.o;P.prototype.transformStyle=P.prototype.j;
