...
var matches = p$1.matches || p$1.matchesSelector ||
...
tree.Composed = {
  ...
  saveChildNodes: function saveChildNodes$2() {
  },
...
var ShadyMixin = {
  ...
  _rendererForHost: function _rendererForHost() {
    ...
    if (isShadyRoot(root)) {
      var c$ = tree.Logical.getChildNodes(this.host);
  ...
  _updateChildNodes: function _updateChildNodes(container, children) {
    var composed;
    if (isShadyRoot(container)) {
      composed = tree.Logical.getChildNodes(container);
    } else {
      var patched = container.__patched;
      if (patched) {
        common.unpatchNode(container);
      }
      composed = tree.arrayCopy(container.childNodes);
      if (patched) {
        common.patchNode(container);
      }
    }
...
var Mixins = {
  ...
  Node: extendAll({__patched: MixinTypes.NODE}, NodeMixin),

  Fragment: extendAll({__patched: MixinTypes.FRAGMENT},
    NodeMixin, FragmentMixin, ActiveElementMixin),

  Element: extendAll({__patched: MixinTypes.ELEMENT},
    NodeMixin, FragmentMixin, ElementMixin, ActiveElementMixin),

  // Note: activeElement cannot be patched on document!
  Document: extendAll({__patched: MixinTypes.DOCUMENT},
...
function saveChildNodes$1$1(node) {
  if (!hasLogical('firstChild')) {
    node.__shady = node.__shady || {};
    node.__shady.firstChild = node.firstChild;
    node.__shady.lastChild = node.lastChild;
    var c$ = node.__shady.childNodes = Array.from(node.childNodes);
    for (var i=0, n; (i<c$.length) && (n=c$[i]); i++) {
      n.__shady = n.__shady || {};
      n.__shady.parentNode = node;
      n.__shady.nextSibling = c$[i+1] || null;
    ...
    for (var i=0; i<ip$.length; i++) {
      ...
      if (this$1._contains(container, insertionPoint)) {
        ...
        for (var j=0; j<dc$.length; j++) {
          hostNeedsDist = true;
          var node = dc$[j];
          var parent = getNative(node, 'parentNode');
          if (parent) {
    ...
    get: function get$3() {
      if (hasLogical(this, 'firstChild')) {
        if (!this.__shady.childNodes) {
          ...
          for (var n=this.firstChild; n; n=this.nextSibling(n)) {
            node.__shady.childNodes.push(n);
          }
        }
...
if (settings.inUse) {
  ...
  for (var p in Mixins.Fragment) {
    var method = Mixins.Fragment[p];
