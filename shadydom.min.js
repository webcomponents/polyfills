...
function innerHTML$1(node) {
  let content = node.localName == 'template' ? node.content : node;
  return getInnerHTML(content, (n) => childNodes$1(n));
}
...
let accessors = {};
const accessorNames = ['parentNode', 'firstChild', 'lastChild',
'previousSibling', 'nextSibling', 'childNodes',
'parentElement', 'previousElementSibling', 'nextElementSibling',
'innerHTML', 'textContent'];

let multiAccessors = ['firstElementChild', 'lastElementChild', 'children'];

accessorNames.push(...multiAccessors);


accessorNames.forEach(name => {
  let proto;
  if (Node.prototype.hasOwnProperty(name)) {
    proto = Node.prototype;
  } else if (Element.prototype.hasOwnProperty(name)) {
    proto = Element.prototype;
  } else if (HTMLElement.prototype.hasOwnProperty(name)) {
    proto = HTMLElement.prototype;
  }
  if (!proto && hasDescriptors$1) {
    throw Error(`Could not find descriptor for ${name}`);
  }
  accessors[name] = Object.getOwnPropertyDescriptor(proto, name);
});

let documentAccessors = {};
let fragmentAccessors = {};
multiAccessors.forEach(name => {
  documentAccessors[name] = Object.getOwnPropertyDescriptor(Document.prototype, name);
  fragmentAccessors[name] = Object.getOwnPropertyDescriptor(DocumentFragment.prototype, name);
});


function parentNode$2(node) {
  return accessors.parentNode.get.call(node);
}

function firstChild$2(node) {
  return accessors.firstChild.get.call(node);
}

function lastChild$2(node) {
  return accessors.lastChild.get.call(node);
}

function previousSibling$2(node) {
  return accessors.previousSibling.get.call(node);
}

function nextSibling$2(node) {
  return accessors.nextSibling.get.call(node);
}

function childNodes$2(node) {
  return accessors.childNodes.get.call(node);
}

function parentElement$2(node) {
  return accessors.parentElement.get.call(node);
}

function firstElementChild$2(node) {
  let desc = accessors.firstElementChild;
  if (node.nodeType == Node.DOCUMENT_NODE && documentAccessors.firstElementChild) {
    desc = documentAccessors.firstElementChild;
  }
  if (node.nodeType == Node.DOCUMENT_FRAGMENT_NODE && fragmentAccessors.firstElementChild) {
    desc = fragmentAccessors.firstElementChild;
  }
  return desc.get.call(node);
}

function lastElementChild$2(node) {
  let desc = accessors.lastElementChild;
  if (node.nodeType == Node.DOCUMENT_NODE && documentAccessors.lastElementChild) {
    desc = documentAccessors.lastElementChild;
  }
  if (node.nodeType == Node.DOCUMENT_FRAGMENT_NODE && fragmentAccessors.lastElementChild) {
    desc = fragmentAccessors.lastElementChild;
  }
  return desc.get.call(node);
}

function previousElementSibling$2(node) {
  return accessors.previousElementSibling.get.call(node);
}

function nextElementSibling$2(node) {
  return accessors.nextElementSibling.get.call(node);
}

function children$2(node) {
  let desc = accessors.children;
  if (node.nodeType == Node.DOCUMENT_NODE && documentAccessors.children) {
    desc = documentAccessors.children;
  }
  if (node.nodeType == Node.DOCUMENT_FRAGMENT_NODE && fragmentAccessors.children) {
    desc = fragmentAccessors.children;
  }
  return desc.get.call(node);
}

function innerHTML$2(node) {
  return accessors.innerHTML.get.call(node);
}

function textContent$2(node) {
  return accessors.textContent.get.call(node);
}
...
let parentNode = hasDescriptors ? parentNode$2 :
  parentNode$1;

let firstChild = hasDescriptors ? firstChild$2 :
firstChild$1;

let lastChild = hasDescriptors ? lastChild$2 :
lastChild$1;

let previousSibling = hasDescriptors ? previousSibling$2 :
previousSibling$1;

let nextSibling = hasDescriptors ? nextSibling$2 :
nextSibling$1;

let childNodes = hasDescriptors ? childNodes$2 :
childNodes$1;

let parentElement = hasDescriptors ? parentElement$2 :
parentElement$1;

let firstElementChild = hasDescriptors ? firstElementChild$2 :
firstElementChild$1;

let lastElementChild = hasDescriptors ? lastElementChild$2 :
lastElementChild$1;

let previousElementSibling = hasDescriptors ? previousElementSibling$2 :
previousElementSibling$1;

let nextElementSibling = hasDescriptors ? nextElementSibling$2 :
nextElementSibling$1;

let children = hasDescriptors ? children$2 :
children$1;

let innerHTML = hasDescriptors ? innerHTML$2 :
innerHTML$1;

let textContent = hasDescriptors ? textContent$2 :
textContent$1;
...
const hasDescriptors$2 = settings.hasDescriptors;
...
let InsideAccessors = {
  ...
  innerHTML: {
    ...
    set(text) {
      ...
      clearNode(content);
      if (hasDescriptors$2) {
        accessors.innerHTML.set.call(htmlContainer, text);
      } else {
