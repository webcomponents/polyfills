...
(function () {
  ...
  (function () {
    ...
    for (var i = 0; i < scripts.length; i++) {
      ...
      if (
        src &&
        src.slice(src.length - 'testharness.js'.length) === 'testharness.js'
      ) {
        ...
        if (
          ...
          switch (val.nodeType) {
            case Node.ELEMENT_NODE:
              ...
              ret +=
                '>' + val.innerHTML + '</' + val.tagName.toLowerCase() + '>';
  ...
  Tests.prototype.notify_start = function () {
    ...
    forEach_windows(function (w, is_same_origin) {
  ...
  Tests.prototype.notify_complete = function () {
    clearTimeout(this.timeout_id);
    var this_obj = this;
    var tests = map(this_obj.tests, function (test) {
  ...
  function assert(
    ...
    if (expected_true !== true) {
      throw new AssertionError(
        make_message(function_name, description, error, substitutions)
      );
    }
  }
