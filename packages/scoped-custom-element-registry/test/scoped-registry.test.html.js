...
describe('Scoped Registry', () => {
  ...
  it('should be able to be shared between different shadowRoots', ()=>{
    ...
    expect(firstShadowRoot.firstElementChild).to.not.be.equal(secondShadowRoot.firstElementChild);
    expect(firstShadowRoot.firstElementChild).to.be.instanceof(CustomElementClass);
    expect(secondShadowRoot.firstElementChild).to.be.instanceof(CustomElementClass);
  });
