ShadyDOM = {
  ...
  noPatch: window.location.search.match('noPatch=on-demand')
    ? 'on-demand'
    : !!window.location.search.match('noPatch'),
...
if (window.customElements) {
  customElements.forcePolyfill = window.location.search.match(
    'forceCustomElements'
  );
}
...
const loadSD = () =>
  loadScript('../node_modules/@webcomponents/shadydom/shadydom.min.js');
